...
class KeyboardView(tk.Canvas):
    ...
    def redraw(self) -> None:
        ...
        padding_x, padding_y = 24, 18
        gap, key_height, row_gap = 9, 62, 9
        ...
        available = max(450, width - padding_x * 2)
        unit = min(82.0, max(48.0, (available - widest_gaps * gap) / widest_units))
        board_width = widest_units * unit + widest_gaps * gap
        total_height = len(self.layout.rows) * key_height + (len(self.layout.rows) - 1) * row_gap
        self.configure(height=total_height + padding_y * 2)
    ...
    def _draw_key(self, key, box) -> None:
        ...
        self.create_text(
            ...
            anchor="w",
            fill=TEXT,
            font=("Segoe UI Semibold", 13 if key.width >= 1 else 11),
        )
        self.create_text(
            x1 + 10,
            y2 - 13,
            text=f"Physical: {key.label}",
            anchor="w",
            fill=MUTED,
